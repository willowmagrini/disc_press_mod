<?php
/** 
 * DiscPressOAuthSimple - A simpler version of OAuth
 *
...
class DiscPressOAuthSimple {
	 ...
	 * @return DiscPressOAuthSimple (Object)
     */
    ...
    public function setURL ($path)
	{
        if (empty($path))
		{
            throw new DiscPressOAuthSimpleException('No path specified for DiscPressOAuthSimple.setURL');
		}
        $this->_path=$path;
    ...
    public function setAction ($action) 
    {
		...
		{
            throw new DiscPressOAuthSimpleException('Invalid action specified for DiscPressOAuthSimple.setAction');
		}
        $this->_action = $action;
    ...
    public function signatures ($signatures) 
    {
		...
		{
            throw new DiscPressOAuthSimpleException('Must pass dictionary array to DiscPressOAuthSimple.signatures');
		}
        if (!empty($signatures))
        {
		...
		{
            throw new DiscPressOAuthSimpleException('Missing required consumer_key in DiscPressOAuthSimple.signatures');
        }
        if (empty($this->_secrets['shared_secret']))
		{
            throw new DiscPressOAuthSimpleException('Missing requires shared_secret in DiscPressOAuthSimple.signatures');
		}
        if (!empty($this->_secrets['oauth_token']) && empty($this->_secrets['oauth_secret']))
		{
            throw new DiscPressOAuthSimpleException('Missing oauth_secret for supplied oauth_token in DiscPressOAuthSimple.signatures');
		}
    ...
    public function setSignatureMethod ($method="")
	{
        ...
        switch($method)
        {
            ...
            case 'HMAC-SHA1':
				...
				break;
            default:
                throw new DiscPressOAuthSimpleException ("Unknown signing method $method specified for DiscPressOAuthSimple.setSignatureMethod");
				break;
        }
    ...
    */
    ...
    private static function _oauthEscape($string) 
    {
        ...
        if (is_array($string)) {
            throw new DiscPressOAuthSimpleException('Array passed to _oauthEscape');
		}
    ...
    private function _getApiKey() 
    {
        if (empty($this->_secrets['consumer_key']))
        {
            throw new DiscPressOAuthSimpleException('No consumer_key set for DiscPressOAuthSimple');
        }
    ...
    private function _getAccessToken() 
    {
		...
		{
            throw new DiscPressOAuthSimpleException('No access token (oauth_token) set for DiscPressOAuthSimple.');
		}
    ...
    private function _generateSignature () 
    {
        ...
        switch($this->_parameters['oauth_signature_method'])
        {
            ...
            default:
                throw new DiscPressOAuthSimpleException('Unknown signature method for DiscPressOAuthSimple');
				break;
        }
    }
}

class DiscPressOAuthSimpleException extends Exception {
